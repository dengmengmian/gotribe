...
type Config struct {
	Model
	ConfigID    string   `gorm:"type:varchar(10);not null;comment:旧版字符ID，保留用于历史数据兼容;" json:"config_id"`
	ProjectID   int64    `gorm:"not null;index;comment:项目ID;" json:"project_id"`
	Alias       string   `gorm:"type:varchar(20);not null;uniqueIndex:idx_config_alias;comment:别名" json:"alias"`
	...
	MDContent   string   `gorm:"type:text;not null;comment:MD内容" json:"md_content"`
...
func (c *Config) BeforeCreate(tx *gorm.DB) error {
	c.ConfigID = utils.GenShortID(utils.WithNumber(10))
