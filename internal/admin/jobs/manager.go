...
func NewCronJobManager(log *zap.SugaredLogger) *CronJobManager {
...
func (m *CronJobManager) Register(job Job) error {
	...
	m.log.Infof("Job %s registered successfully", jobName)
	return nil
}
...
func (m *CronJobManager) Start() error {
	...
	m.cron.Start()
	m.log.Info("Job scheduler started")
	return nil
}
...
func (m *CronJobManager) Stop() error {
	...
	ctx := m.cron.Stop()
	<-ctx.Done()
	m.log.Info("Job scheduler stopped")
	return nil
}
...
func (m *CronJobManager) EnableJob(jobName string) error {
	...
	m.entryIDs[jobName] = entryID
	m.log.Infof("Job %s enabled", jobName)
	return nil
}
...
func (m *CronJobManager) DisableJob(jobName string) error {
	...
	m.log.Infof("Job %s disabled", jobName)
	return nil
}
...
func (m *CronJobManager) executeJob(jobName string) {
	...
	if !exists {
		m.log.Errorf("Job %s not found", jobName)
		return
	}
	...
	if err != nil {
		...
		m.log.Errorf("Job %s failed: %v", jobName, err)
	} else {
		result.Status = StatusCompleted
		result.Message = "Job completed successfully"
		m.log.Infof("Job %s completed in %v", jobName, result.Duration)
	}
...
func (m *CronJobManager) executeWithRetry(ctx context.Context, job Job, result *JobResult) error {
	...
	for i := 0; i <= job.RetryCount(); i++ {
		...
		if err := job.Execute(ctx); err != nil {
			lastErr = err
			if i < job.RetryCount() {
				m.log.Warnf("Job %s failed (attempt %d/%d), retrying: %v",
