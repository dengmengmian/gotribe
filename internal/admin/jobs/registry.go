...
func NewJobRegistry(manager JobManager, log *zap.SugaredLogger) *JobRegistry {
	return &JobRegistry{
		manager: manager,
...
func (r *JobRegistry) Register(job Job) error {
	...
	r.jobs[jobName] = job
	r.log.Infof("Job %s registered successfully", jobName)
	return nil
}
...
// InitGlobalRegistry 初始化全局任务注册表
func InitGlobalRegistry(log *zap.SugaredLogger) *JobRegistry {
	registryOnce.Do(func() {
		manager := NewCronJobManager(log)
		globalRegistry = NewJobRegistry(manager, log)
	})
